...
class DisplibSolver:
    ...
    def add_path_constraints(self):
        for i, train in tqdm(enumerate(self.trains), desc="Adding path-constraints"):
            ...
            self.model.add(
                sum(self.edge_select_vars[i][out_edge] for out_edge in self.train_graphs[i].out_edges(0)) == 1)
            self.model.add(
                sum(self.edge_select_vars[i][in_edge] for in_edge in self.train_graphs[i].in_edges(last_op)) == 1)
        ...
        for i, train in tqdm(enumerate(self.trains), desc="Adding timing-constraints"):
            for op in range(len(self.train_graphs[i].nodes)):
                for in_edge in self.train_graphs[i].in_edges(op):
                    ...
                    self.model.add(self.op_start_vars[i, in_edge[0]] + self.trains[i][in_edge[0]]["min_duration"] <=
                                   self.op_start_vars[i, op]).OnlyEnforceIf(self.edge_select_vars[i][in_edge])

                for out_edge in self.train_graphs[i].out_edges(op):
                    # Operation ends when successor operation starts
                    self.model.add(self.op_end_vars[i, op] == self.op_start_vars[i, out_edge[1]]).OnlyEnforceIf(
                        self.edge_select_vars[i][out_edge])
    ...
    def solve(self):
        ...
        if status == cp.OPTIMAL or status == cp.FEASIBLE:
            ...
            return Log(status, round(self.solver.best_objective_bound),
                       Solution(round(self.solver.objective_value), events))
        else:
            ...
            while v != len(graph.nodes) - 1:
                for succ in self.trains[i][v]["successors"]:
                    ...
                    if self.solver.value(self.edge_select_vars[i][(v, succ)]) == 1:
                        ...
                        events.append(
                            {"time": self.solver.value(self.op_start_vars[(i, v)]), "train": i, "operation": v})
                        break
    ...
    def topological_sort(self, events):
        sorted_events = []
        lhs, rhs = 0, 0
    ...
    def create_topological_graph(self, events):
        ...
        for i, event in enumerate(events):
            ...
            needed_resources = [res["resource"] for res in
                                self.trains[event["train"]][event["operation"]]["resources"]]
            ...
            for j, other in enumerate(events):
                ...
                predecessor_resources = [res["resource"] for res in
                                         self.trains[other["train"]][predecessor]["resources"]]
        ...
        for i, event in enumerate(events):
            ...
            if duration > 0:
                ...
                critical_resources.append(
                    r["resource"] for r in self.trains[event["train"]][event["operation"]]["resources"])
            else:
